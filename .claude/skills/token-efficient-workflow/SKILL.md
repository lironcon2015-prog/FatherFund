...
- **הבדיקות רצות במלואן** (`node --test tests/*.test.mjs`) לפני מיזוג ל-`main`,
  ולוגיקה חדשה ב-`engine.js`/`rules.js` מגיעה עם טסט.
- **קידום גרסה בכל המקומות** שמפורטים ב-`CLAUDE.md` (כולל `?v=` ב-`index.html`).
- **ניתוח השפעה לפני מחיקה או refactor**, ו"הסבר למה" כשמציעים שינוי — זה לא פטפוט.
- **תיעוד ההכרעות ב-`CLAUDE.md` מתעדכן** כשכלל משתנה. מה שנחתך הוא הנרטיב בצ׳אט.
- **סיכום קצר בעברית בסוף כל משימה.**
